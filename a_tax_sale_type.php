<?php
    ...
    if(isset($conn)) {
        ...
        mysqli_query($conn,"SET NAMES UTF8");
            ...
            $str_sql = "SELECT * FROM `tax_sale_type_tb`";
            $obj_rs = mysqli_query($conn,$str_sql);

            $arr_result['list'] = array();
            while($obj_row = mysqli_fetch_array($obj_rs)) {
                $arr_data = array();
                $arr_data['ajax_tax_sale_type_code'] = $obj_row['tax_sale_type_code'];
                $arr_data['ajax_tax_sale_type_name1'] = $obj_row['tax_sale_type_name1'];
